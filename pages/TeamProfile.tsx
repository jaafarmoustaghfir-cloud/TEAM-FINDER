...
import { 
  MapPin, Calendar, Info, ShieldCheck, Mail, 
  Camera, X, Phone, Lock, Navigation, ExternalLink, Map as MapIcon
} from 'lucide-react';
    ...
    <div className="min-h-screen pb-24 bg-zinc-950">
        ...
        <div className="max-w-7xl mx-auto px-4 h-full flex items-end pb-16 relative z-10">
          <div className="flex flex-col md:flex-row items-center md:items-end gap-10 w-full animate-in fade-in slide-in-from-bottom-6">
            ...
            <div className="flex gap-4">
               ...
               {isOwner && (
                  <button onClick={() => setIsLocationModalOpen(true)} className="px-10 py-5 bg-zinc-800 text-white rounded-[2rem] font-black uppercase text-xs tracking-widest shadow-xl hover:bg-zinc-700 transition-all">Update Venue</button>
               )}
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-8 space-y-24">
          <section id="contact-details">
            <h2 className="text-3xl font-black text-white mb-10 uppercase tracking-tighter flex items-center gap-4">
              <Phone className="w-8 h-8 text-blue-500" /> Club Registry
            </h2>
               ...
               {!isContactRevealed ? (
                 ...
                 <div className="animate-in fade-in zoom-in-95">
                    ...
                    <div className="mt-12 flex justify-center">
                       <button onClick={() => setIsContactRevealed(false)} className="text-zinc-500 hover:text-white font-black uppercase text-[10px] tracking-[0.2em] transition-colors">Close Session</button>
                    </div>
                 </div>
               )}
            </div>
           ...
           <div className="bg-zinc-900 border border-zinc-800 rounded-[3.5rem] p-10 sticky top-32 shadow-2xl">
              ...
              <div className="bg-zinc-950 p-8 rounded-[2rem] border border-zinc-800 mb-8">
                 ...
                 {teamData.locationLink && (
                    <a href={teamData.locationLink} target="_blank" className="inline-flex items-center gap-2 text-blue-400 text-[10px] font-black uppercase tracking-widest hover:text-white transition-colors">
                      Open in Maps <ExternalLink className="w-3 h-3" />
                    </a>
                 )}
              </div>
              {isOwner && (
                <button onClick={() => setIsLocationModalOpen(true)} className="w-full py-5 bg-zinc-800 hover:bg-zinc-700 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-lg transition-all">Update Venue Link</button>
              )}
           </div>
        </aside>
      </div>
      ...
      {isLocationModalOpen && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-xl bg-zinc-900 border border-zinc-800 rounded-[3rem] p-12 shadow-2xl animate-in zoom-in-95">
            <div className="flex items-center justify-between mb-10">
               <div>
                  <h3 className="text-3xl font-black text-white uppercase tracking-tighter">Venue Upload</h3>
                  <p className="text-zinc-500 text-[10px] font-black uppercase tracking-widest mt-1">Provide Name and Maps link</p>
               </div>
            ...
            <form onSubmit={handleUpdateLocation} className="space-y-8">
               <div>
                  <label className="text-zinc-500 text-[10px] uppercase font-black mb-3 block tracking-widest">Team Name / Venue Name</label>
                  <input type="text" value={locationInput} onChange={(e) => setLocationInput(e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 rounded-2xl p-6 text-white outline-none focus:border-blue-600 font-bold transition-all" placeholder="e.g. London FC Academy Hub" required />
               </div>
               <div>
                  <label className="text-zinc-500 text-[10px] uppercase font-black mb-3 block tracking-widest">Google Maps Link or GPS</label>
                  <input type="text" value={linkInput} onChange={(e) => setLinkInput(e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 rounded-2xl p-6 text-white outline-none focus:border-blue-600 font-bold transition-all" placeholder="https://maps.google.com/..." required />
               </div>
               <button type="submit" className="w-full py-6 bg-blue-600 hover:bg-blue-500 text-white rounded-[2rem] font-black uppercase tracking-widest shadow-2xl active:scale-95 transition-all">Submit Location</button>
            </form>
          </div>
        </div>
      )}
