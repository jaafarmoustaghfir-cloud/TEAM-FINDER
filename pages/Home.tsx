...
import { 
  User, Shield, Search, ArrowRight, Zap, 
  Target, Sparkles, Video, Star, Users, 
  MessageSquare, ShieldCheck, MapPin,
  Trophy, TrendingUp, PlayCircle
} from 'lucide-react';
...
const Home: React.FC = () => {
  ...
  // Simulated logged-in users for demonstration of the matching engine
  ...
  // RECOMMENDATION ALGORITHM: Matches players to teams based on Position, Level, and Needs
  ...
  const recommendedPlayers = useMemo(() => {
    ...
    return MOCK_PLAYERS.filter(player => {
      ...
      return hasVideos && posMatch && player.isApproved;
    }).slice(0, 4);
  }, [role, currentTeam]);
      ...
      <div className="flex flex-col gap-24 pb-24 bg-zinc-950">
        <section className="relative h-[90vh] flex items-center justify-center overflow-hidden px-4">
          ...
          <div className="relative z-10 max-w-7xl mx-auto text-center animate-in fade-in zoom-in-95 duration-1000">
            <div className="inline-flex items-center gap-3 px-6 py-2.5 rounded-[1.5rem] bg-orange-500/10 border border-orange-500/20 text-orange-500 text-[10px] font-black uppercase tracking-[0.3em] mb-12 shadow-2xl">
              <Zap className="w-4 h-4 fill-current" />
              Professional Scouting Engine
            </div>
            <h1 className="text-7xl md:text-[9rem] font-black text-white mb-10 leading-[0.85] tracking-tighter uppercase">
              Find Your Team.<br />
            ...
            <p className="text-xl md:text-3xl text-zinc-500 max-w-4xl mx-auto mb-16 leading-relaxed font-medium px-4">
              The premier digital infrastructure for local talent and professional recruitment. Verified profiles, verified footage.
            </p>

            <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
              <Link to="/register?role=player" className="px-14 py-7 bg-orange-600 hover:bg-orange-500 text-white rounded-[2.5rem] font-black text-xl transition-all flex items-center gap-4 active:scale-95 shadow-2xl uppercase tracking-widest">
                <User className="w-6 h-6" /> I am a Player
              </Link>
              <Link to="/register?role=team" className="px-14 py-7 bg-zinc-900 border border-zinc-800 hover:border-zinc-700 text-white rounded-[2.5rem] font-black text-xl transition-all flex items-center gap-4 active:scale-95 uppercase tracking-widest shadow-2xl">
        ...
        <section className="max-w-7xl mx-auto px-4 w-full grid grid-cols-1 md:grid-cols-3 gap-10">
           {[
             ...
             <button 
                ...
                className="group text-left bg-zinc-900 border border-zinc-800 p-12 rounded-[3.5rem] hover:border-orange-500/40 transition-all shadow-xl active:scale-95 relative"
             >
               <div className="absolute top-8 right-8 p-3 bg-zinc-950 rounded-2xl group-hover:bg-orange-600 transition-colors">
               ...
               <div className="text-white text-[10px] font-black uppercase tracking-widest flex items-center gap-2 group-hover:text-orange-500 transition-colors">
                  Login to Activate
               </div>
             </button>
           ))}
  ...
  return (
    <div className="flex flex-col gap-24 pb-24 pt-12 bg-zinc-950 min-h-screen">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-10">
          <div>
            <div className="flex items-center gap-3 text-orange-500 font-black uppercase tracking-[0.3em] text-[10px] mb-4">
              <Sparkles className="w-4 h-4" /> Professional Scouter Active
            </div>
            ...
            <p className="text-zinc-500 mt-4 font-medium text-lg">
              {role === 'player' ? 'Complete your "Skills Videos" to become visible to official scouts.' : 'Review personalized player matches based on your club needs.'}
            </p>
          </div>
          
          <div className="flex gap-4">
            <button onClick={() => handleFeatureClick('video')} className="bg-zinc-900 border border-zinc-800 hover:border-zinc-700 text-white px-8 py-4 rounded-[1.5rem] font-black uppercase text-[10px] transition-all flex items-center gap-3 shadow-xl">
              <Video className="w-5 h-5" /> {role === 'player' ? 'Skills Videos' : 'Scout Hub'}
            </button>
            <button onClick={() => handleFeatureClick('matching')} className="bg-white text-zinc-950 hover:bg-zinc-200 px-8 py-4 rounded-[1.5rem] font-black uppercase text-[10px] transition-all flex items-center gap-3 shadow-2xl">
      ...
      <section className="max-w-7xl mx-auto px-4 w-full grid grid-cols-1 md:grid-cols-3 gap-10">
           {[
             { id: 'video', title: 'Video Portfolio', desc: role === 'player' ? 'Upload match highlights. (MP4, WEBM). Required for club recommendations.' : 'Review verified player footage from top candidates.', icon: <Video className="w-10 h-10 text-orange-500" /> },
             { id: 'matching', title: 'Matching Engine', desc: 'Discovery matches based on position, level, and location logic.', icon: <Target className="w-10 h-10 text-orange-500" /> },
             { id: 'access', title: 'Scouting Access', desc: 'Secure contact registry for trial invites and recruitment inquiries.', icon: <MessageSquare className="w-10 h-10 text-orange-500" /> }
           ].map((feature, i) => (
             <button key={i} onClick={() => handleFeatureClick(feature.id as any)} className="group text-left bg-zinc-900 border border-zinc-800 p-12 rounded-[3.5rem] hover:border-blue-500/40 transition-all shadow-xl active:scale-95">
               <div className="mb-8">{feature.icon}</div>
               <h3 className="text-2xl font-black text-white uppercase tracking-tighter mb-4">{feature.title}</h3>
               <p className="text-zinc-500 font-medium leading-relaxed mb-6">{feature.desc}</p>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
            <div>
              ...
              <h2 className="text-5xl font-black text-white uppercase tracking-tighter leading-none">
                {role === 'player' ? 'Matching Vacancies' : 'Scouting Targets'}
              </h2>
            </div>
            <Link to="/search" className="text-zinc-400 hover:text-white text-[10px] font-black uppercase tracking-widest flex items-center gap-2">View Full Directory <ArrowRight className="w-4 h-4" /></Link>
          </div>
            ...
            {role === 'player' ? (
              recommendedTeams.length > 0 ? (
                ...
                <div className="col-span-full py-20 bg-zinc-900/50 border-2 border-dashed border-zinc-800 rounded-[3rem] text-center">
                   <p className="text-zinc-500 font-black uppercase tracking-widest text-[10px]">No matches found. Try updating your position in settings.</p>
                </div>
              )
            ) : (
          ...
          <div className="max-w-4xl relative z-10">
            ...
            <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-6">
              <input 
                ...
                placeholder={role === 'team' ? "Find strikers, goalkeepers, defenders..." : "Find pro, semi-pro, or academy clubs..."}
